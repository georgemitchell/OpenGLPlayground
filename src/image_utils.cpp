...
bool WriteJpeg( const char * destinationFile, const unsigned char * rgbxBuffer, int width, int height )
{
	...
	const int r = tjCompress2( 
					tj, 
					( unsigned char * )rgbxBuffer,	// TJ isn't const correct...
					width, 
					width * 4, 
					height, 
					TJPF_RGBX, 
...
unsigned char * TurboJpegLoadFromMemory( const unsigned char * jpg, const int length, int * width, int * height )
{
	...
	int jpegSubsamp;
	int jpegColorspace;
	const int headerRet = tjDecompressHeader3( 
							tj,
							( unsigned char * )jpg /* tj isn't const correct */, 
							length, 
							&jpegWidth, 
							&jpegHeight,
							&jpegSubsamp, 
	...
	if ( buffer != NULL )
	{
		const int decompRet = tjDecompress2( 
								tj,
								( unsigned char * )jpg, 
								length, 
								buffer,
